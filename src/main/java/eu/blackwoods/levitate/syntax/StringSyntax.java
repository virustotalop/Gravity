...
public class StringSyntax implements SyntaxHandler {
	...
	public void check(String parameter, String passed) throws SyntaxResponseException 
	{
		...
		if(parameter.equals("") || parameter.equals("aA")) {
			if(isInt(passed)) 
				throw new SyntaxResponseException(Message.STRINGSYNTAX_CANNOT_BE_INT.get(TextMode.COLOR, replaces));
			return;
		}
		if(parameter.equals("a")) {
			if(!isLowerCase(passed)) 
				throw new SyntaxResponseException(Message.STRINGSYNTAX_ONLY_LOWERCASE.get(TextMode.COLOR, replaces));
		}
		if(parameter.equals("A")) {
			if(!isUpperCase(passed)) 
				throw new SyntaxResponseException(Message.STRINGSYNTAX_ONLY_UPPERCASE.get(TextMode.COLOR, replaces));
		}
		return;
	}
