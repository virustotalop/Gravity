	...
	public void check(String parameter, String passed) throws CommandSyntaxException, SyntaxResponseException 
	{
		...
		if(parameter.equals("+")) 
		{
			if(passedInt < 0 && passedInt != 0) 
				throw new SyntaxResponseException(Message.INTEGERSYNTAX_HAS_TO_BE_POSITIVE_ZERO.get(TextMode.COLOR, replaces));
		} 
		else if(parameter.equals("-")) 
		{
			if(passedInt > 0 && passedInt != 0) 
				throw new SyntaxResponseException(Message.INTEGERSYNTAX_HAS_TO_BE_NEGATIVE_ZERO.get(TextMode.COLOR, replaces));
		} 
		else if(parameter.contains("-")) 
		{
			replaces.put("%parameter%", parameter);
			if(!parameter.matches("(-?[0-9]+)-(-?[0-9]+)")) 
				throw new CommandSyntaxException(Message.INTEGERSYNTAX_PARAMETER_MALFORMED.get(TextMode.COLOR, replaces));
			...
			else 
			{
				...
				replaces.put("%max%", String.valueOf(b));
				if(a > b) 
					throw new SyntaxResponseException(Message.INTEGERSYNTAX_PARAMETER_MALFORMED_2.get(TextMode.COLOR, replaces));
				if(passedInt < a || passedInt > b) 
					throw new SyntaxResponseException(Message.INTEGERSYNTAX_HAS_TO_BE_INBETWEEN.get(TextMode.COLOR, replaces));
			}		
		}
	}
